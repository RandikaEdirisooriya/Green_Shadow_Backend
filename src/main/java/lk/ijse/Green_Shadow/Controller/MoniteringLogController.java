...
public class MoniteringLogController {
    ...
    @GetMapping("/nextcode")
    public ResponseUtill getNewLogsCode() {
        String newCode = AppUtil.generateLogCode(moniteringLogService.findLastLogsCode());
        return new ResponseUtill("Success", "Retrieved New Code", newCode);
    }
    @GetMapping("/count")
    public ResponseEntity<Long> getCount() {
        long Count = moniteringLogService.getCount();
        return new ResponseEntity<>(Count, HttpStatus.OK);
    }
    @GetMapping("/ids")
    public ResponseEntity<List<String>> getAllIds() {
        List<String> Ids = moniteringLogService.getAllIds();
